...
class ImageResizeComponent extends Component
{
    ...
    function resize($imagePath, $destinationWidth, $destinationHeight, $thumb_name)
    {
        ...
        if(file_exists($imagePath)) {
            ...
            if($mimeType == 'image/jpeg' || $mimeType == 'image/pjpeg') {
                ...
                imagejpeg($destination, $thumb_name);
            } else if($mimeType == 'image/gif') {
                $source = imagecreatefromgif($imagePath);
                imagecopyresampled($destination, $source, 0, 0, 0, 0, $destinationWidth, $destinationHeight, $sourceWidth, $sourceHeight);
                imagegif($destination, $thumb_name);
            } else if($mimeType == 'image/png' || $mimeType == 'image/x-png') {
                $source = imagecreatefrompng($imagePath);
                imagecopyresampled($destination, $source, 0, 0, 0, 0, $destinationWidth, $destinationHeight, $sourceWidth, $sourceHeight);
                imagepng($destination, $thumb_name);
            } else {
